...
st.set_page_config(page_title="Lavo e Levo V28 - Smart", page_icon="🧺", layout="wide")

# Estilos CSS com o Sistema de Cores
st.markdown("""
    <style>
    ...
    .card-verde { border-left-color: #28a745; } /* Verde: No prazo */
    .card-amarelo { border-left-color: #ffc107; } /* Amarelo: Atenção */
    .card-vermelho { border-left-color: #dc3545; } /* Vermelho: Atrasado */
...
def gerar_romaneio_pdf(row):
    ...
    for item in str(row['detalhe_itens']).split(','):
        p.drawString(120, y, f"• {item.strip()}"); y -= 20
...
if st.sidebar.button("🔄 Sincronizar / Atualizar Tempos"):
    st.cache_data.clear(); st.rerun()

df, conn = carregar_dados()
if df is None:
    st.error("⚠️ Falha ao ler a planilha. Verifique a URL ou aguarde 5 min.")
    st.stop()
...
with tab2:
    ...
    if not espera.empty:
        ...
        lotes = st.multiselect("Selecione os Lotes:", espera['id'].tolist(), format_func=lambda x: f"{df[df['id']==x]['cli'].values} ({df[df['id']==x]['p_in'].values}kg)")
...
with tab3:
    ...
    for i, row in ativos.iterrows():
        ...
        # Lógica de Cores
        cor_classe = "card-verde" if minutos <= 30 else "card-amarelo" if minutos <= 60 else "card-vermelho"
        
        with st.container():
            st.markdown(f"<div class='status-card {cor_classe}'>", unsafe_allow_html=True)
